...
export function parseUser(userContainer: Element): PlayerData | null {
  ...
  const headerBar = userContainer.querySelector(
  ...
  const cardRow = userContainer.querySelector(
    'div[style*="align-self: center"]'
  );
  if (cardRow) {
    for (const cardDiv of Array.from(cardRow.children)) {
      const textContainer = cardDiv.querySelector(
        'div[style*="align-items: center;"]'
      );
      if (!textContainer) continue;

      const countElement =
        textContainer.firstElementChild as HTMLElement | null;
      const changeElement =
        countElement?.nextElementSibling as HTMLElement | null;
