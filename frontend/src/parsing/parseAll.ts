...
import { parseBidOffer } from "./bidOffer";
import { parseSingleTrade } from "./singleTrade";
...
export function getTrades(): FullTrade[] | null | [] {
  ...
  tradeRows.forEach((row) => {
    const single = parseSingleTrade(row as HTMLDivElement);
    const bid = parseBidOffer(row as HTMLDivElement, true);
    const offer = parseBidOffer(row as HTMLDivElement, false);

    const thisTrade: FullTrade = {
      bid: bid,
      offer: offer,
      trade: single,
    };
