 ...
 */
export function parseTradeRow(tradeRowWrapper: HTMLDivElement): BidOfferData[] | null {
  ...
  const buyerNameElement = columns[0].querySelector('div[dir="auto"]') as HTMLElement | null;
  const buyer = {
  ...
  const sellerNameElement = columns[2].querySelector('div[dir="auto"]') as HTMLElement | null;
  const seller = {
